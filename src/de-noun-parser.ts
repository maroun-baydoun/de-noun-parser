...
export const parse = (wikitext: string): ParseResult => {
  ...
  const templateArguments = wikitext.substring(
    startIndex + "{{de-noun|".length,
    endIndex
  );
  ...
  if (["f", "m", "n"].indexOf(baseTokens[0]) < 0) {
    throw new UnrecognisedGenderError(
      `Expected m, f or n, but recieved ${baseTokens[0]}`
    );
  }

  const gender = baseTokens[0] as Gender;
  const genetive =
    baseTokens[1] || (gender === "m" || gender === "n" ? "-s" : null);
  const plural = baseTokens[2] || "-en";
  const diminutive = baseTokens[3] || null;
  const genderedForms = additionalTokens.filter(
    (token) => token.indexOf("f=") !== -1 || token.indexOf("m=") !== -1
  );
  const genderedForm = genderedForms.length
    ? genderedForms[0].split("=")[1]
    : null;
